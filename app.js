...
const elements = {
  ...
  maxDeviationCard: $("max-deviation-card"), maxDeviation: $("max-deviation"),
};
...
function headingUpPoint(vector, heading, scale) {
  ...
  return { x: TRACK_CENTER + right * scale, y: TRACK_CENTER - forward * scale };
}
...
function render() {
  ...
  if (nearest && state.maxDeviation !== null) {
    const accuracy = Number.isFinite(state.accuracy) ? state.accuracy : 0;
    const adjustedDeviation = Math.max(0, nearest.distance - accuracy);
    state.maxDeviation = Math.max(state.maxDeviation, adjustedDeviation);
  }
  ...
  elements.maxDeviationCard.hidden = state.maxDeviation === null;
  if (state.maxDeviation !== null) elements.maxDeviation.textContent = formatDistance(state.maxDeviation);
